...
from agents import CATEAgentConfig, CATEAgent
...
@dataclass
class Options(CausalMnistBanditsConfig, CATEAgentConfig):
  seed: int = 5000
  debug: bool = False
    ...
    with tqdm(total=config.num_ts) as pbar:
        while not timestep.done:
            ...
            if timestep.id % config.telemetry_every == 0:
                vis.collect_cate(agent, mnist_env, timestep)
